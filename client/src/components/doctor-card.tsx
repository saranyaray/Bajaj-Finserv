...
const DoctorCard = ({ doctor }: DoctorCardProps) => {
  const hasPhoto = doctor.photo && typeof doctor.photo === 'string';
    ...
    >
      <div className="flex-shrink-0 flex flex-col items-center">
        <div className="w-24 h-24 rounded-full bg-neutral-100 overflow-hidden mb-2 flex items-center justify-center">
          ...
          <span className={`text-3xl font-bold text-primary ${hasPhoto ? 'hidden' : ''}`}>
      ...
      <div className="flex-1">
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-2">
          <div>
            ...
            <p data-testid="doctor-specialty" className="text-sm text-neutral-400">
              {doctor.specialty ? doctor.specialty.join(", ") : doctor.specialities?.map(s => s.name).join(", ")}
            </p>
          </div>
